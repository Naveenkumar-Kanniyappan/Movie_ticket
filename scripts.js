...
postMovieForm.addEventListener("submit", (e) => {
  ...
  let movieGenreInput = document.getElementById("movieGenre").value.trim();
  let movieLanguage = document.getElementById("movieLanguage").value.trim();
  let moviePoster = document.getElementById("moviePoster").value.trim();
  let movieReleaseDate = document.getElementById("movieReleaseDate").value.trim();
  let movieRating = parseFloat(document.getElementById("movieRating").value);

  if (!movieTitle || !movieGenreInput || !movieLanguage || !moviePoster || !movieReleaseDate || isNaN(movieRating)) {
    console.error("Please fill in all required fields.");
    return;
  }

  let movieGenreArray = movieGenreInput.split(",").map((genre) => genre.trim().toLowerCase());
  ...
  for (let i = 0; i < movieGenreArray.length; i++) {
    let genreId = genreSelect[movieGenreArray[i]];
    if (genreId) {
  ...
  if (genreIds.length === 0) {
    alert("No valid genres selected. Please check the genre names and try again.");
    return;
  }
  ...
  request.onload = function () {
    if (request.status === 201) {
      ...
      let genre = document.createElement("p");
      genre.textContent = "Genre: " + movieGenreArray.join(", ");
      genre.style.textAlign = "center";
      genre.style.color = "black";
      imgcontainer.appendChild(genre);
